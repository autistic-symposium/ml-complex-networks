...
FOLDERS_OUT = ['m_s5000', 'm_s500', 's_n4']
FOLDERS_IN  = ['m_s5000/', 'm_s500/', 's_n4/']
...
def parse_feature_vector(fraw):
    ...
    for line in open(fraw,'r'):
        line = line.strip('\n')
        linea = line.split(': ')
        if linea[0] != '' and linea[0] > 2 and len(linea)>1 and not math.isnan(float(linea[1])):
...
if __name__ == '__main__':
    ...
    for i in range(len(FOLDERS_IN)):
        ...
        for folder in SUBFOLDERS:
            ...
            OUTPUT_PATH =  'vectors/' + folder + '_' + FOLDERS_OUT[i] + '.data'
        ...
        for folder in SUBFOLDERS:
            ...
            INPUT_PATH = FOLDERS_IN[i] + folder + '/'
            OUTPUT_PATH =  'vectors/' + folder + '_' + FOLDERS_OUT[i] + '.data'

            # verify whether the folder exist
            print INPUT_PATH
            try:
                os.stat(INPUT_PATH)
                ...
                print('Folder ' + INPUT_PATH + ' does not exist.')
